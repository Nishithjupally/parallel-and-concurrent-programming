...
public:
			...
			if(comparePointer(check_tail,q->tail.load()))
			{
				if(check_next.ptr==NULL)
				{
					...
					if((q->tail.load().ptr->next).compare_exchange_strong(check_next, temp))
					{
						...
						(q->tail.load()).compare_exchange_strong(check_tail, temp2);
						return;
					}
					check_next = check_tail.ptr->next.load();
				...
				else
				{
					...
					(q->tail.load()).compare_exchange_strong(check_tail, temp1);
				}
    ...
    int dequeue(queue_t* q)
    {
        while(1)
        {
            pointer_t check_head = q->head;
            pointer_t check_tail = q->tail;
            pointer_t check_next = check_head.ptr->next.load();

            if(check_head == q->head.load())
            {
                if(check_head.ptr == check_tail.ptr)
                {
                    ...
                    while((check_next.ptr->next.load().ptr != NULL) && comparePointer(q->tail.load(),check_tail)z)
                    {
                        check_next = check_next.ptr->next.load();
                    }
                    //CAS
